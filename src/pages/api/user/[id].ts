...
const handlerUserEditInfo = async (request: NextApiRequest, response: NextApiResponse): Promise<void> => {
    ...
    if (request.method === 'PUT') {
        ...
        try {
            ...
            const updatedUser = await db.collection('user_card').findOne({$or: [{name}]});
            ...
            /* 변경 내용 DB 저장 */
            await db.collection('user_card').updateOne({ _id: new ObjectId(id as string) }, { $set: { name: name, birth: birth } });

            /* Response */
            return response.status(200).json({message: 'Information has been successfully changed.'});
